...
function BlogPost({
  ...
  const handleDelete = async () => {
    try {
      const userDeets = JSON.parse(localStorage.getItem("auth_user"));
      const currentUserId = userDeets.id
      //Get the authentication token from local storage
      const token = userDeets.token;
      ...
      const response = await fetch(apiUrl, {
        //Send API request to delete post
        method,
        headers: {
  ...
  return (
    ...
    <article className={`${styles.blogPost} ${isDarkMode ? styles.dark : ""}`}>
      ...
      <div
        className={`${styles.blogPost__content} ${isExpanded ? styles.expanded : "" // Apply expanded class if isExpanded is true
          }`}
      >
